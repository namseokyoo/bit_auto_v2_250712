...
# 프로세스 시작 시간 기록 (전역 변수)
PROCESS_START_TIME = datetime.now(KST)

def get_uptime():
    """운영 시간 계산"""
    now = datetime.now(KST)
    uptime = now - PROCESS_START_TIME
    days = uptime.days
    ...
    <script>
        ...
        function showUpbitTradeDetails(trade) {
            ...
            details.innerHTML = `
                ...
                <div class="metric">
                    <span class="metric-label">체결 가격:</span>
                    <span class="metric-value">₩${Math.floor(trade.price || 0).toLocaleString()}</span>
                </div>
                <div class="metric">
                    <span class="metric-label">체결 수량:</span>
                    <span class="metric-value">${(trade.quantity || 0).toFixed(8)} BTC</span>
                </div>
                <div class="metric">
                    <span class="metric-label">거래 금액:</span>
                    <span class="metric-value">₩${Math.floor(trade.trade_amount || 0).toLocaleString()}</span>
                </div>
                <div class="metric">
                    <span class="metric-label">수수료:</span>
                    <span class="metric-value">₩${Math.floor(trade.fee || 0).toLocaleString()}</span>
                </div>
                <div class="metric">
                    <span class="metric-label">정산 금액:</span>
                    <span class="metric-value" style="font-weight: bold;">₩${Math.floor(trade.settlement_amount || 0).toLocaleString()}</span>
                </div>
                <div class="metric">
            ...
            try {
                ...
                fetch('/api/upbit-trades')
                            ...
                            const tradesHtml = tradesData.trades.map(trade => {
                                ...
                                return `
                                    <div class="trade-item ${sideClass}" onclick='showUpbitTradeDetails(${JSON.stringify(trade).replace(/'/g, "&apos;")})' style="cursor: pointer;">
                                        ...
                                        <span class="trade-price">₩${settlementAmount.toLocaleString()}</span>
                    ...
                    .catch(error => {
                        ...
                        document.getElementById('recent-trades').innerHTML = '<div class="trade-item">거래 내역 로드 실패</div>';
                    });
...
@app.route('/api/upbit-trades')
def get_upbit_trades():
    """Upbit API를 통한 실제 거래내역 조회"""
    try:
        upbit = pyupbit.Upbit(
            os.getenv('UPBIT_ACCESS_KEY'),
            os.getenv('UPBIT_SECRET_KEY')
        ...
        # 최근 체결 내역 조회 (pyupbit는 get_order로 개별 조회만 가능)
        # 따라서 DB 거래와 Upbit 잔고 정보를 조합
        conn = sqlite3.connect('data/quantum.db')
        cursor = conn.cursor()
        ...
        trades = []
        for row in cursor.fetchall():
            timestamp, side, price, quantity, fee, pnl, signal_strength = row
            
            # 거래금액 계산 (수수료 반영)
            if side == 'BUY':
                # 매수: 거래금액 + 수수료
                trade_amount = (price * quantity) + fee
                settlement_amount = trade_amount  # 정산금액 = 거래금액 + 수수료
            else:
                # 매도: 거래금액 - 수수료
                trade_amount = price * quantity
                settlement_amount = trade_amount - fee  # 정산금액 = 거래금액 - 수수료
        ...
        logger.error(f"Error fetching Upbit trades: {e}")
        return jsonify({'error': str(e)}), 500
