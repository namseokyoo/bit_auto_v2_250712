...
class AIPerformanceAnalyzer:
    ...
    def analyze_strategy_performance(self, strategy_id: str, 
        ...
        try:
            ...
            if len(executions) < self.min_trades_for_analysis:
                self.logger.warning(f"분석을 위한 최소 거래 수({self.min_trades_for_analysis}) 미달: {len(executions)}")
                return None
            ...
            calmar_ratio = abs(average_pnl) / max_drawdown if max_drawdown > 0 else 0
            ...
            best_hour = max(hourly_performance.items(), key=lambda x: x[1])[0] if hourly_performance else 0
            worst_hour = min(hourly_performance.items(), key=lambda x: x[1])[0] if hourly_performance else 0
            avg_execution_time = df['execution_duration'].mean() if 'execution_duration' in df.columns else 0
            ...
            optimization_priority = self._determine_optimization_priority(ai_score, total_trades)
    ...
    def generate_optimization_recommendations(self, 
        ...
        for metrics in strategy_metrics:
            try:
                # 전략별 권장사항 생성
                strategy_recommendations = self._analyze_strategy_for_optimization(metrics)
    ...
    def get_portfolio_performance_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            portfolio_volatility = np.std(strategy_returns) if len(strategy_returns) > 1 else 0
            portfolio_sharpe = self._calculate_sharpe_ratio(strategy_returns) if strategy_returns else 0
    ...
    def _calculate_signal_accuracy(self, df: pd.DataFrame) -> float:
        ...
        buy_accuracy = len(buy_signals[buy_signals['pnl'] > 0]) / len(buy_signals) if len(buy_signals) > 0 else 0
        sell_accuracy = len(sell_signals[sell_signals['pnl'] > 0]) / len(sell_signals) if len(sell_signals) > 0 else 0
        ...
        weighted_accuracy = (buy_accuracy * len(buy_signals) + sell_accuracy * len(sell_signals)) / total_signals
    ...
    def _calculate_execution_efficiency(self, df: pd.DataFrame) -> float:
        ...
        if len(high_confidence_signals) > 0:
            high_conf_execution_rate = len(high_confidence_signals[high_confidence_signals['trade_executed'] == True]) / len(high_confidence_signals)
        else:
    ...
    def _calculate_ai_optimization_score(self, win_rate: float, sharpe_ratio: float, 
        ...
        # 트렌드 보너스/페널티
        trend_bonus = {'improving': 5, 'stable': 0, 'declining': -5, 'insufficient_data': 0}
        trend_score = trend_bonus.get(performance_trend, 0)
        
        total_score = win_rate_score + sharpe_score + drawdown_score + accuracy_score + trend_score
        ...
        try:
            ...
            if abs(metrics.best_performance_hour - metrics.worst_performance_hour) > 2:
                recommendations.append(OptimizationRecommendation(
                    ...
                    parameters={'preferred_hours': [metrics.best_performance_hour]}
                ))
