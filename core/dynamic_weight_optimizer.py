...
class DynamicWeightOptimizer:
    ...
    def optimize_portfolio_weights(self, 
        ...
        with self._lock:
            try:
                self.logger.info(f"포트폴리오 가중치 최적화 시작: {optimization_method.value}")
    ...
    def apply_weight_adjustments(self, optimization_result: PortfolioOptimizationResult) -> bool:
        ...
        try:
            if optimization_result.confidence < self.min_confidence:
                self.logger.warning(f"신뢰도가 낮아 조정 적용 취소: {optimization_result.confidence:.3f}")
                return False
            ...
            if success:
                ...
                for adjustment in optimization_result.adjustments:
                    self.logger.info(f"  {adjustment.strategy_id}: {adjustment.old_weight:.3f} → {adjustment.new_weight:.3f}")
    ...
    def auto_optimize_weights(self) -> Optional[PortfolioOptimizationResult]:
        ...
        try:
            # 성과 기반 최적화 실행
            result = self.optimize_portfolio_weights(WeightAdjustmentStrategy.PERFORMANCE_BASED)
    ...
    def _get_current_weights(self) -> Dict[str, float]:
        """현재 전략 가중치 조회"""
        try:
            weights = config_manager.get_config('independent_strategies.strategy_weights', {})
            
            # 기본값 설정
            if not weights:
                active_strategies = self._get_active_strategies()
                weights = {strategy_id: 1.0 for strategy_id in active_strategies}
    ...
    def _performance_based_weights(self, strategy_metrics: Dict[str, AdvancedPerformanceMetrics]) -> Dict[str, float]:
        ...
        if total_score > 0:
            for strategy_id, score in scores.items():
                ...
                weights[strategy_id] = max(min(weight, self.max_weight), self.min_weight)
    ...
    def _risk_adjusted_weights(self, strategy_metrics: Dict[str, AdvancedPerformanceMetrics]) -> Dict[str, float]:
        ...
        for strategy_id, metrics in strategy_metrics.items():
            ...
            drawdown_penalty = max(1 - (metrics.max_drawdown / 1000), 0.1)  # 낙폭 페널티
    ...
    def _normalize_weights(self, weights: Dict[str, float]) -> Dict[str, float]:
        ...
        for strategy_id, weight in weights.items():
            ...
            normalized[strategy_id] = max(min(normalized_weight, self.max_weight), self.min_weight)
    ...
    def _evaluate_optimization_result(self, 
        ...
        # 예상 수익률 계산
        original_return = self._calculate_portfolio_return(original_weights, strategy_metrics)
        optimized_return = self._calculate_portfolio_return(optimized_weights, strategy_metrics)
        
        # 예상 리스크 계산
        original_risk = self._calculate_portfolio_risk(original_weights, strategy_metrics)
        optimized_risk = self._calculate_portfolio_risk(optimized_weights, strategy_metrics)
    ...
    def _calculate_portfolio_risk(self, weights: Dict[str, float], 
        ...
        for strategy_id, weight in weights.items():
            if strategy_id in strategy_metrics:
                ...
                weighted_variance = ((weight / total_weight) ** 2) * (strategy_volatility ** 2)
    ...
    def _calculate_optimization_confidence(self, 
        ...
        # 1. 데이터 충분성 (거래 수 기반)
        avg_trades = np.mean([m.total_trades for m in strategy_metrics.values()])
        ...
        for strategy_id in original_weights:
            if strategy_id in optimized_weights:
                change = abs(optimized_weights[strategy_id] - original_weights[strategy_id])
        ...
        # 3. 성과 일관성
        performance_scores = [m.ai_optimization_score for m in strategy_metrics.values()]
        ...
        for strategy_id in optimized_weights:
            ...
            if abs(new_weight - old_weight) > 0.05:  # 5% 이상 변화만 기록
                ...
                adjustment = WeightAdjustment(
                    ...
                    expected_improvement=abs((new_weight - old_weight) / old_weight) * 10,
