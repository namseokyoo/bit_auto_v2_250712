...
class AutoTrader:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not config_manager.is_trading_enabled():
                self.logger.warning("자동거래가 비활성화되어 있습니다.")
                return False
            
            # 데이터베이스 확인
            try:
                db.get_trades(limit=1)  # 간단한 DB 연결 테스트
                self.logger.info("데이터베이스 연결 확인됨")
            except Exception as e:
    ...
    def _setup_schedule(self):
        ...
        if interval_minutes >= 60:
            ...
            for hour in range(0, 24, interval_minutes // 60):
                schedule.every().day.at(f"{hour:02d}:00").do(self._execute_auto_trading)
        else:
            # 분 단위 스케줄
            schedule.every(interval_minutes).minutes.do(self._execute_auto_trading)
    ...
    def stop(self):
        ...
        # 중지 로그 기록
        db.insert_log('INFO', 'AutoTrader', '자동거래 중지', f'중지 시간: {datetime.now(self.kst)}')
    ...
    def _execute_auto_trading(self):
        ...
        try:
            ...
            if self.trading_engine:
                ...
                self.logger.info(f"✅ 자동거래 실행 완료 - 성공률: {self.state.successful_executions}/{self.state.total_executions}")
    ...
    def _validate_market_data(self) -> bool:
        ...
        try:
            ...
            current_price = self.trading_engine.api.get_current_price("KRW-BTC")
    ...
    def _pre_trade_risk_check(self) -> bool:
        ...
        try:
            ...
            if self.state.failed_executions > 5:
                ...
                last_success_time = getattr(self.state, 'last_success_time', None)
                if last_success_time:
                    time_since_success = (datetime.now(self.kst) - last_success_time).total_seconds()
