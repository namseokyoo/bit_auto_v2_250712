...
class ParameterLogger:
    ...
    def log_parameter_adjustment(self, regime_result: RegimeResult,
        ...
        try:
            ...
            self._append_to_json_log(
                self.parameter_log_file, asdict(log_entry))
    ...
    def log_batch_adjustments(self, regime_result: RegimeResult,
        ...
        try:
            ...
            self.logger.info(
                f"=== 파라미터 일괄 조정 시작 ({regime_result.primary_regime.value}) ===")
            ...
            # 체제 변경 로깅
            self.log_regime_change(
                regime_result, affected_strategies, total_adjustments)
