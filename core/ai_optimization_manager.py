...
class AIOptimizationManager:
    ...
    def execute_full_optimization(self, mode: OptimizationMode = None) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_summary = ai_performance_analyzer.get_portfolio_performance_summary(30)
            ...
            if self._should_run_weight_optimization(health_metrics):
                ...
                optimization_results['weight_optimization'] = weight_result.to_dict() if weight_result else None
            ...
            self.logger.info(f"전체 최적화 완료: {optimization_summary.get('overall_improvement', 0):.1f}% 개선")
    ...
    def configure_optimization(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if 'auto_enabled' in config:
                ...
                self.logger.info(f"자동 최적화: {'활성화' if self.auto_optimization_enabled else '비활성화'}")
            
            if 'interval_hours' in config:
                self.optimization_interval = max(1, int(config['interval_hours']))
    ...
    def _evaluate_system_health(self) -> SystemHealthMetrics:
        ...
        try:
            # 포트폴리오 성과 요약
            portfolio_summary = ai_performance_analyzer.get_portfolio_performance_summary(30)
            ...
            # 리스크 점수 (높을수록 안전)
            strategies_needing_optimization = portfolio_summary.get('strategies_needing_optimization', 0)
            total_strategies = portfolio_summary.get('total_strategies', 1)
            
            risk_score = max(100 - (strategies_needing_optimization / total_strategies * 100), 0)
    ...
    def _create_optimization_summary(self, optimization_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            post_health = optimization_results.get('post_optimization_health', {})
            
            if pre_health and post_health:
                health_improvement = post_health.get('overall_score', 0) - pre_health.get('overall_score', 0)
            ...
            if trading_result and trading_result.get('success'):
                ...
                applied_count = trading_result.get('applied_recommendations', 0)
                if applied_count > 0:
                    summary['overall_improvement'] += applied_count * 5
                    summary['key_changes'].append(f'{applied_count}개 거래 파라미터 최적화')
            ...
            if tuning_result and tuning_result.get('success'):
                ...
                total_optimizations = tuning_result.get('total_optimizations', 0)
                if total_optimizations > 0:
                    summary['overall_improvement'] += total_optimizations * 3
                    summary['key_changes'].append(f'{total_optimizations}개 전략 파라미터 튜닝')
            
            # 권장사항 생성
            if summary['overall_improvement'] < 5:
                summary['recommendations'].append('시스템 성과가 안정적입니다')
            elif summary['overall_improvement'] > 20:
                summary['recommendations'].append('상당한 개선이 이루어졌습니다. 성과를 모니터링하세요')
