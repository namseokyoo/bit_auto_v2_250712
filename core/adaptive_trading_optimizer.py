...
class AdaptiveTradingOptimizer:
    ...
    def analyze_market_condition(self) -> Optional[MarketAnalysis]:
        ...
        try:
            ...
            self.logger.info(f"시장 분석 완료: {market_condition.value}, 신뢰도: {confidence:.3f}")
            return analysis
    ...
    def generate_optimization_recommendations(self, 
        ...
        try:
            # 1. 거래 주기 최적화
            trading_interval_rec = self._optimize_trading_interval(market_analysis)
            if trading_interval_rec:
                recommendations.append(trading_interval_rec)
            
            # 2. 신뢰도 임계값 최적화
            confidence_threshold_rec = self._optimize_confidence_threshold(market_analysis)
            ...
            # 5. 전략 민감도 조정
            sensitivity_rec = self._optimize_strategy_sensitivity(market_analysis)
            if sensitivity_rec:
    ...
    def apply_optimization_recommendations(self, 
        ...
        for rec in recommendations:
            try:
                ...
                if success:
                    self.logger.info(f"{rec.parameter.value} 최적화 적용: {rec.current_value} → {rec.recommended_value}")
                else:
    ...
    def auto_optimize(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 2. 최적화 권장사항 생성
            recommendations = self.generate_optimization_recommendations(market_analysis)
            if not recommendations:
                return {'success': True, 'message': '최적화 권장사항 없음', 'market_condition': market_analysis.condition.value}
            
            # 3. 권장사항 적용
            application_results = self.apply_optimization_recommendations(recommendations)
            ...
            result = {
                ...
                'recommendations': [rec.to_dict() for rec in recommendations[:3]]  # 상위 3개만
            }
            
            self.logger.info(f"자동 최적화 완료: {applied_count}/{len(recommendations)} 적용")
            return result
    ...
    def _collect_market_data(self) -> Optional[Dict]:
        ...
        try:
            ...
            # 다양한 시간대 캔들 데이터 수집
            candles_1h = api.get_candles('KRW-BTC', minutes=60, count=24)  # 24시간
            candles_5m = api.get_candles('KRW-BTC', minutes=5, count=288)  # 24시간 5분봉
            
            if not candles_1h or not candles_5m:
                return None
            
            # 현재가 정보
            ticker = api._make_request('GET', '/v1/ticker', {'markets': 'KRW-BTC'})
            current_price = float(ticker[0]['trade_price']) if ticker and len(ticker) > 0 else None
    ...
    def _analyze_volume_profile(self, market_data: Dict) -> float:
        ...
        try:
            ...
            volumes = [float(candle['candle_acc_trade_volume']) for candle in candles[-50:]]
    ...
    def _analyze_support_resistance(self, market_data: Dict) -> float:
        ...
        try:
            ...
            current_price = market_data.get('current_price', (max(highs) + min(lows)) / 2)
            
            # 현재가가 범위의 중간에 가까울수록 지지/저항이 강함
            mid_price = (max(highs) + min(lows)) / 2
            distance_from_mid = abs(current_price - mid_price) / (price_range / 2) if price_range > 0 else 0
    ...
    def _calculate_analysis_confidence(self, market_data: Dict) -> float:
        ...
        data_sufficiency = min((candles_1h_count / 24 + candles_5m_count / 288) / 2, 1.0)
    ...
    def _optimize_trading_interval(self, market_analysis: MarketAnalysis) -> Optional[OptimizationRecommendation]:
        """거래 주기 최적화"""
        try:
            current_interval = config_manager.get_config('trading.trade_interval_minutes', 10)
            
            # 시장 상황에 따른 최적 주기 결정
            if market_analysis.condition == MarketCondition.HIGH_VOLATILITY:
                recommended_interval = max(self.min_trading_interval, current_interval // 2)  # 빠른 거래
                reason = "고변동성 시장에서 빠른 대응을 위해 거래 주기 단축"
                improvement = 15.0
            elif market_analysis.condition == MarketCondition.LOW_VOLATILITY:
                recommended_interval = min(self.max_trading_interval, current_interval * 2)  # 느린 거래
                ...
                recommended_interval = max(self.min_trading_interval, int(current_interval * 0.8))  # 약간 빠르게
    ...
    def _optimize_confidence_threshold(self, market_analysis: MarketAnalysis) -> Optional[OptimizationRecommendation]:
        """신뢰도 임계값 최적화"""
        try:
            current_threshold = config_manager.get_config('independent_strategies.buy_threshold', 0.6)
            
            # 시장 상황에 따른 임계값 조정
            if market_analysis.condition == MarketCondition.HIGH_VOLATILITY:
                recommended_threshold = min(current_threshold + 0.1, 0.8)  # 더 엄격하게
                reason = "고변동성 시장에서 거래 신뢰도 임계값 상향 조정"
                improvement = 8.0
            elif market_analysis.condition == MarketCondition.LOW_VOLATILITY:
                recommended_threshold = max(current_threshold - 0.1, 0.4)  # 더 관대하게
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if recent_market_analyses:
                ...
                avg_volatility = np.mean([a.volatility for a in recent_market_analyses])
                avg_trend_strength = np.mean([a.trend_strength for a in recent_market_analyses])
            else:
